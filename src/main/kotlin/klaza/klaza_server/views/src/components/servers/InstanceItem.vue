	...
	>
        <template v-slot:header>
			<div class="fit row no-wrap items-center">
				...
				<span class="text-bk text-ellipsis col"> {{ course.fullName }} </span>
				<span class="text-bk text-ellipsis col"> {{ instance.channel_id }} </span>
				<span class="text-bk text-ellipsis col"> {{ instance.creator.username }}</span>
        ...
        <q-card>
			<q-card-section class="configs">
				<course-configs :course="course" :type="type" :instance="instance" />
			</q-card-section>
        </q-card>
    </q-expansion-item>
</template>
...
import { CourseDTO, UserCourseDiscordConfigDTO, UserCourseTelegramConfigDTO } from "src/@types/dtos";
...
export default defineComponent({
    ...
    props: {
        ...
        type: {
            type: String as () => "discord_user" | "discord_other" | "telegram_user" | "telegram_other",
            required: true,
        },
    },
	methods: {
